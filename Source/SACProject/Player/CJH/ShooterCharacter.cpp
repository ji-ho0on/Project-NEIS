...
{
	if (m_Aiming)
		m_CameraCurrentFOV = FMath::FInterpTo(m_CameraCurrentFOV,
												m_CameraZoomedFOV,
												DeltaTime,
												m_ZoomInterpSpeed);
	else
...
void AShooterCharacter::UpdateCrosshairSpread(float DeltaTime)
{
	...
	// 조준 시 적용
	if (m_Aiming)
		m_CrosshairAimingFactor = FMath::FInterpTo(m_CrosshairAimingFactor, 1.0f, DeltaTime, 20.0f);
	else
	...
	// 총합 Crosshair spread mulitlier 계산
...
void AShooterCharacter::Stun()
{
	...
	{
		SetShooterState(EShooterState::Stunned);
...
void AShooterCharacter::Tick(float DeltaTime)
{
	...
	{
		ADefaultPlayerState* DefaultPlayerState = GetPlayerState<ADefaultPlayerState>();
...
void AShooterCharacter::GetPickupItem(AShooterItem* InItem)
{
	...
	if (IsValid(HealPack))
	{
		// HealPack일 경우 HealPercent에 따라 체력 회복
		ADefaultPlayerState* DefaultPlayerState = GetPlayerState<ADefaultPlayerState>();
		if (IsValid(DefaultPlayerState))
		{
...
void AShooterCharacter::WeaponFire()
{
	...
	{
		if (m_EquippedWeapon->WeaponFire())
		{
			UAnimInstance* AnimInst = GetMesh()->GetAnimInstance();
			if (IsValid(AnimInst) && IsValid(m_FireMontage))
			{
				AnimInst->Montage_Play(m_FireMontage);
				AnimInst->Montage_JumpToSection(FName("StartFire"));
			}
		}
	}
}
...
{
	if (GetShooterState() == EShooterState::Stunned || !GetCharacterMovement()->IsFalling())
	{
		m_Crouching = true;
	}
	if (m_Crouching)
	...
	if (GetShooterState() == EShooterState::Stunned || !GetCharacterMovement()->IsFalling())
	{
		...
		if (IsValid(DefaultPlayerState))
		{
			if (DefaultPlayerState->GetMP() >= 50)
			{
				...
				LaunchCharacter(Velocity * m_DashSpeed, false, false);
