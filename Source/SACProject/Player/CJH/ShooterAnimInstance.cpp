...
void UShooterAnimInstance::UpdateAnimationProperties(float DeltaSeconds)
{
	...
	if (m_ShooterCharacter)
	{
		...
		m_Reloading = m_ShooterCharacter->GetShooterState() == EShooterState::Reloading;
		m_Equipping = m_ShooterCharacter->GetShooterState() == EShooterState::Equipping;
...
void UShooterAnimInstance::TurnInPlace()
{
	...
	else
	{
		...
		// RootYawOffset을 업데이트하고, NormalizeAxis를 통해 -180 ~ 180 사이의 각도로 클램핑
		m_RootYawOffset = UKismetMathLibrary::NormalizeAxis(m_RootYawOffset - YawDeltaTIP);

		// Turning이라는 Curve가 추가된 애니메이션이 실행될 때 CurveValue는 1.0f를 반환하고, 아니라면 0.0f를 반환한다.
